...
| #34 | ~15:00 25/09/2026 | "giờ tôi sẽ quay 5 video và nếu video nào tìm ra lỗi tôi sẽ nói bạn và bạn tạo log Github issues cho tôi nhé" *(kèm ảnh tem thiết bị và ảnh chụp test case của Gemini)* |
| #35 | ~18:40 25/09/2026 | "bạn hãy làm cho tôi đi" *(kèm kết quả em tự ghi sau khi quay video: TC01, TC02, TC09 Pass; TC15 và TC10 Fail)* |
| #36 | ~19:05 25/09/2026 | "Giữ nguyên, không tụt, Đọc rõ hết, không nhòe, không bong, Chỉ có tiếng gió, không lạch cạch, cấp 3 ồn nhất, Không thấy tia lửa, chạy lại ngay, Dây lành, phích chắc, không xê dịch, Đồng xu còn nguyên, không lệch" *(kết quả quan sát của em cho các test case còn lại)* |
...
| #38 | ~19:25 25/09/2026 | "tôi vừa tạo repo git hub rồi cho tôi đoạn 3 issue đó để tôi dán nó vào đi" *(kèm 5 link YouTube)* |
...
| #50 | ~22:45 25/09/2026 | "giờ bỏ lỗi DEF-02 đi và tôi đã sủa 03 thành 02 rồi và các file dox trong template tôi có kí rồi mà phục hồi prompt log lại cho tôi prompt nào có dòng nhắc không đặt vào log thì khỏi cần bỏ vào" *(kèm ảnh trang GitHub Issues sau khi sửa và ảnh file [AI-03] đã ký)* |

### Thay đổi sau phiên

Ở thời điểm prompt #38 và #40 em đang có **3 defect** và đã tạo 3 issue. Sau khi rà lại (prompt #50), defect "lồng bảo vệ tháo được bằng tay" bị rút khỏi danh sách: Expected Result của TC14 viện dẫn TCVN 5699-2-80 nhưng không tra được nguyên văn điều khoản, nên không có test oracle để phán quyết. Issue tương ứng đã xoá và DEF-03 được đánh số lại thành DEF-02. Kết quả cuối: **2 defect, 2 issue**. Các prompt #38 và #40 ở trên giữ nguyên văn tại thời điểm gửi, không sửa lại theo kết quả cuối.
...
Một số prompt trong Session 05 không được ghi vào log. Đó là những prompt mà ngay trong nội dung em có yêu cầu Claude không ghi lại, và Claude đã làm theo. Em ghi nhận đây là chỗ log chưa đạt yêu cầu "mọi prompt đều được ghi" của đề.

Tuy vậy **không có sản phẩm AI nào không được khai báo**: toàn bộ 15 artifact do AI sinh ra — kể cả những artifact sinh ra từ các prompt không ghi — đều có mặt trong `[AI-02]` kèm prompt nguyên văn ở cột (1), verdict và phần sinh viên sửa. Phần thiếu là dòng thời gian trong log, không phải phần khai báo.
